package br.com.graphtheoryufcg.a03;
...
public class Questao01 {
  ...
  /**
   * Imports a graph and finds the nodes with most Betweeness Centrality
   * and Closeness Centrality, and calculates the assotativity of the graph.
   *
   * @param args
   */
  public static void main(String[] args) {

    final String path = "./input/a03/q01.gml";

    Graph graph = JGraphUtil.importGraphGML(path);

    printSpreaders(graph);
    printInfluencers(graph);
    printRessemblance(graph);
  }

  /**
   * Print the nodes with the highest closeness centrality.
   * @param graph the graph
   */
  private static void printSpreaders(Graph graph) {
    Map<String, Double> centralities = JGraphUtil.getClosenessCentrality(graph);

    String result = "- Disseminadores: " + "\n" + getResultsStringFromMap(centralities);
  ...
  /**
   * Print the nodes with the highest betweeness centrality.
   * @param graph
   */
  private static void printInfluencers(Graph graph) {
    Map<String, Double> centralities = JGraphUtil.getBetweenessCentrality(graph);

    String result = "- Influenciadores: " + "\n" + getResultsStringFromMap(centralities);

    System.out.println(result + "\n");
  }

  /**
   * Prints the assortativity of a graph.
   * @param graph
   */
  private static void printRessemblance(Graph graph) {
    System.out.println("- Semelhança: " + "\n" + JGraphUtil.assortativityCoefficient(graph));
  }

  private static String getResultsStringFromMap(Map<String, Double> centralities) {
    return centralities.entrySet()
            .stream()
            .sorted(getReverseEntryComparator())
            .limit(5)
            .map(getEntryStringFormatFunction())
            .collect(Collectors.joining("\n"));
  }

  private static Function<Entry<String, Double>, String> getEntryStringFormatFunction() {
    return a -> String.format("%5s -> %f", a.getKey(), a.getValue());
  }

  private static Comparator<Entry<String, Double>> getReverseEntryComparator() {
